...
new CfncompatProviderFunctions(string ProviderLocalName);
```
...
```csharp
private string[] Cidr(string IpBlock, double Count, double CidrBits)
```
...
```csharp
private IResolvable ConditionAnd((bool|IResolvable)[] Conditions)
```
...
```csharp
private IResolvable ConditionContains(string[] ListOfStrings, string Value)
```
...
```csharp
private IResolvable ConditionEachMemberEquals(string[] ListOfStrings, string Value)
```
...
```csharp
private IResolvable ConditionEachMemberIn(string[] StringsToCheck, string[] StringsToMatch)
```
...
```csharp
private IResolvable ConditionEquals(object Value1, object Value2)
```
...
```csharp
private IResolvable ConditionIf(bool|IResolvable Condition, object ValueIfTrue = null, object ValueIfFalse = null)
```
...
```csharp
private IResolvable ConditionNot(bool|IResolvable Condition)
```
...
```csharp
private IResolvable ConditionOr((bool|IResolvable)[] Conditions)
```
...
```csharp
private IResolvable FindInMap(object Mapping, string TopLevelKey, string SecondLevelKey, object[] DefaultValue)
```
...
```csharp
private double LengthOf(object Value)
```
...
```csharp
private IResolvable Select(double Index, object Objects)
```
...
```csharp
private string[] Split(string Delimiter, string Source)
```
...
```csharp
private string Sub(string Template, System.Collections.Generic.IDictionary<string, string> Variables)
```
